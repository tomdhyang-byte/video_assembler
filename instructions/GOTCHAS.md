...
- **解法**：
    ...
    2. 實作「標點跳過演算法」。
    3. **教訓**：永遠不要信任 LLM 會逐字輸出，必須在程式碼層級做容錯。
...
- **解法**：除非更換 Avatar 來源影片，否則不要修改 `AVATAR_` 開頭的設定參數。

## 4. API 大小限制
- **現象**：Whisper API 回傳 413 Payload Too Large。
- **原因**：OpenAI Whisper API 限制檔案大小為 25MB。
- **解法**：目前的 `_step1` 流程尚未實作自動切分。若遇到長音檔，需先用 `pydub` 等工具切分後再送出（目前尚未實作，為已知限制）。
